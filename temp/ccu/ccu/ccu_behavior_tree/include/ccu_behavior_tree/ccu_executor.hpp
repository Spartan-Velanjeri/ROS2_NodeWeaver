 ...
 * @brief CcuExecutor feedback utilities required to get transforms and reference frames.
 */
struct FeedbackUtils
{
  int x;
...
/**
 * @class ExecutionMuxer
 * @brief A class to control the state of the BT execution by allowing only a single
...
public:
  /**
   * @brief A Execution Muxer constructor
   */
  ExecutionMuxer()
   ...
   */
  void startExecuting(const std::string & navigator_name)
  {
    std::scoped_lock l(mutex_);
   ...
   */
  void stopExecuting(const std::string & navigator_name)
  {
    std::scoped_lock l(mutex_);
...
/**
 * @class CcuExecutor
 * @brief CcuExecutor interface that acts as a base class for all BT-based CcuExecutor action's plugins
 */
template<class ActionT>
class CcuExecutor
{
public:
  using Ptr = std::shared_ptr<ccu_behavior_tree::CcuExecutor<ActionT>>;

  /**
   * @brief A CcuExecutor constructor
   */
  CcuExecutor()
  {
  ...
  bool on_configure(
    ...
    ccu_behavior_tree::ExecutionMuxer * plugin_muxer)
  {
    ...
    node->declare_parameter("groot_zmq_publisher_port", 1768);
    node->declare_parameter("groot_zmq_server_port", 1769);
    ...
    bt_action_server_ = std::make_unique<nav2_behavior_tree::BtActionServer<ActionT>>(
      ...
      std::bind(&CcuExecutor::onGoalReceived, this, std::placeholders::_1),
      std::bind(&CcuExecutor::onLoop, this),
      std::bind(&CcuExecutor::onPreempt, this, std::placeholders::_1),
      std::bind(&CcuExecutor::onCompletion, this, std::placeholders::_1));
    ...
    blackboard->set<int>("subtract", feedback_utils.x);
  ...
  // virtual std::string getDefaultBTFilename() = 0;
...
protected:
  ...
  bool onGoalReceived(typename ActionT::Goal::ConstSharedPtr goal)
  {
    if (plugin_muxer_->isExecuting()) {
      RCLCPP_ERROR(
        logger_,
        "Requested Execution from %s while another execution is processing,"
  ...
  rclcpp::Logger logger_{rclcpp::get_logger("CcuExecutor")};
  rclcpp::Clock::SharedPtr clock_;
  FeedbackUtils feedback_utils_;
  ExecutionMuxer * plugin_muxer_;
};
