...
# frame_to=rpm_imu
# tmux send-keys -t ${WINDOW}.2 C-z "source ${ROS_PATH} && ros2 run tf2_ros static_transform_publisher ${x} ${y} ${z} ${yaw} ${pitch} ${roll} ${frame_from} ${frame_to}" Enter
...
frame_from=map
frame_to=odom
tmux send-keys -t ${WINDOW}.3 C-z "source ${ROS_PATH} && ros2 run tf2_ros static_transform_publisher ${x} ${y} ${z} ${yaw} ${pitch} ${roll} ${frame_from} ${frame_to}" Enter
