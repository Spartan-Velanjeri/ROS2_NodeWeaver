...
tmux send-keys -t ${WINDOW_CCU}.0 "source ${ROS_PATH} && ros2 launch bautiro_description robot.launch.py robot_name:=FUS2 rpm_name:=RPM2" Enter
...
tmux send-keys -t ${WINDOW_RPM_1OF2}.0 "source ${ROS_PATH} && ros2 launch rpm_sensors rpm_sensors_launch.py mode:=real hw_sample:=RPM2 config_file_with_path:=$(ros2 pkg prefix --share rpm_sensors)/config/RPM2/rpm_config.yaml" Enter
tmux send-keys -t ${WINDOW_RPM_1OF2}.1 "source ${ROS_PATH} && ros2 launch rpm_localization rpm_localization_launch.py mode:=real hw_sample:=RPM2 map:=$(ros2 pkg prefix --share lu_rough_localization)/maps/rosbag_le131" Enter
# tmux send-keys -t ${WINDOW_RPM_1OF2}.1 "source ${ROS_PATH} && ros2 launch rpm_localization rpm_localization_launch.py mode:=real hw_sample:=RPM2 map:=$(ros2 pkg prefix --share lu_rough_localization)/maps/rosbag_le131_AUTOMATED_GEN" Enter
