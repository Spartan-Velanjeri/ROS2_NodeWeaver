...
def launch_setup(context, *args, **kwargs):
    ...
    robot_description_content = Command(
        [
            ...
            PathJoinSubstitution(
                [FindPackageShare(description_package), "robots", robot_name, description_file]
            ),
            " ",
            "safety_limits:=",
            "true",
    ...
    joint_state_broadcaster_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["joint_state_broadcaster", "--controller-manager", "/rpm/controller_manager"],
    )

    diff_base_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["diff_base_controller", "-c", "/rpm/controller_manager"],
    )
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "ros2_control_config_file",
            default_value="rpm_ros2_control.yaml",
            description="Path to ros2_control configuration.",
        )
    )
    declared_arguments.append(
