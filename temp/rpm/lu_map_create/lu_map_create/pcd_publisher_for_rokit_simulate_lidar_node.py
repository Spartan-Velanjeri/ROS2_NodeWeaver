...
class PCDPublisher(Node):
    ...
    def __init__(self):
        super().__init__('pcd_publisher_for_rokit_simulate_lidar_node')
        ...
        self.pose_publisher    = self.create_publisher(geometry_msgs.PoseStamped, 'pose', 100)
        self.pcd_publisher     = self.create_publisher(sensor_msgs.PointCloud2, 'scan', 100)
    ...
    def run(self):
        """run"""
        self.get_logger().info('Generating PCDs ...')
        for i in range(self.nr_of_steps):
            ...
            pcd = self.point_cloud(points_new, self.frame_id_scan)

            self.offset_list.append(offset)
            self.pcd_list.append(pcd)

            self.get_logger().info('generated pcd with offset no. %2i of %2i [%5.2f,%5.2f,%5.2f]'%(i+1,self.nr_of_steps,offset[0],offset[1],offset[2]))


    def publish(self):
        """publish"""
        self.get_logger().info('Piblishing PCDs with %.1f Hz ...'%(1.0/self.publish_timeinterval))

        # version_msg = std_msgs.Int64()
        # version_msg.data = self.map_version
        # self.version_publisher.publish(version_msg)

        for pcd,offset in zip(self.pcd_list,self.offset_list):

            pcd.header.stamp = Clock().now().to_msg()
            ...
            self.get_logger().info('published with offset [%5.2f,%5.2f,%5.2f]'%(offset[0],offset[1],offset[2]))
