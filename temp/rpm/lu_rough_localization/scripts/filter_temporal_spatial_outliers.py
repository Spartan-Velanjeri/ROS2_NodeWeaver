...
class PcdFilter(Node):
    ...
    def __init__(self):
        ...
        self.closest_neighbor_distance = self.get_parameter('closest_neighbor_distance').value
        ...
        #     LaserScan, self.tpc_in.replace('points','scan'), self.filter_pcd_2, self.buffer)
    ...
    def callback_filter_pcd(self, msg):
        ...
        assert len(msg.fields) >= 4, \
            "The pointcloud should contain at least x,y,z,intensity"
        ...
        if not hasattr(self, 'ring_buffer'):
            ...
            self.pcd_out__np_ = np.zeros((n, n_fields), dtype=np.float32)
        ...
        max_dist = np.max(self.pcd_out__np[:, 0:3].__abs__(), axis=1)

        sel_pre = self.pcd_out__np > 0
        sel = np.logical_or(np.logical_or(sel_pre[:, 0], sel_pre[:, 1]), sel_pre[:, 2])
        sel = np.logical_and(sel, max_dist < self.max_distance)

        pcd_out__np_ = self.pcd_out__np[sel]

        dist = np.max(np.abs(np.diff(pcd_out__np_[:, 0:3], axis=0)), axis=1) / \
            (max_dist[sel])[1:]
        ...
        if is_visualize:
            ...
            pcd_out__np_ = np.array(list(self.read_points(pcd_msg)))
        ...
        if self.computation_cnt % 40 == 0:
            self.get_logger().info(
                "Running with %.1fms ..." % (self.computation_time * 1e3))
            self.computation_cnt = 0
