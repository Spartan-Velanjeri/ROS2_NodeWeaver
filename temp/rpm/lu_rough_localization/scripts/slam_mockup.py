...
class FramePublisher(Node):
    ...
    def broadcast_timer_callback(self):
        self.make_transforms_quat(
            'map', 'leica',
            -10.0, 0.0, 0.5,
            0.0, 0.0, 0.0, 1.0)

    def make_transforms(
            self, parent_frame_id, frame_id, x_trans,
            y_trans, z_trans, roll, pitch, yaw):
    ...
    def make_transforms_quat(
            self, parent_frame_id, frame_id, x_trans, y_trans,
            z_trans, quat_x, quat_y, quat_z, quat_w):
        static_transformStamped = TransformStamped()
        static_transformStamped.header.stamp = self.get_clock().now().to_msg()
        static_transformStamped.header.frame_id = parent_frame_id
        static_transformStamped.child_frame_id = frame_id
