...
def copy_root_stuff_into_doc(doc_path):
    ...
    for doc_sibling in scandir(integration_repo_path):
        if doc_sibling.name != 'doc' and not doc_sibling.name.startswith('.'):
            ...
            if doc_sibling.is_file() and \
                    doc_sibling.name not in [
                        'dependencies.repos', 'jenkinsfile_bautiro', 'readme.md']:
                if not exists(dirname(t)):
                    makedirs(dirname(t))
                copy2(s, t)
            else:
                copy_files(s, t, DOC_KEYWORD)
...
def copy_files(src, dst, keyword: str):
    for root, dirs, files in walk(src):
        for file in files:
            if condition_fulfilled(keyword, root, file):
...
def condition_fulfilled(keyword: str, root, file):
