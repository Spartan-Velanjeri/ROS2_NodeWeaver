...
{
public:
  using Ptr = std::shared_ptr<fpm_behavior_tree::Navigator<ActionT>>;
  ...
  bool on_configure(
    ...
    fpm_behavior_tree::NavigatorMuxer * plugin_muxer)
  {
    ...
    // node->declare_parameter("groot_zmq_publisher_port", 1998);
    // node->declare_parameter("groot_zmq_server_port", 1999);
...
protected:
  ...
  rclcpp::Logger logger_{rclcpp::get_logger("FPM Navigator")};
